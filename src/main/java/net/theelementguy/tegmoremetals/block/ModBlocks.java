...
public class ModBlocks {
	...
	public static <T extends Block> DeferredBlock<T> registerBlockWithItem(String name, Supplier<T> block) {
		DeferredBlock<T> toReturn = BLOCKS.register(name, block);
		ModItems.ITEMS.registerSimpleBlockItem(name, toReturn, new Item.Properties().setId(ModUtil.createItemResourceKey(name)));
		return toReturn;
	}

	public static DeferredBlock<Block> registerOre(String name) {
		return registerBlockWithItem(name, () -> new Block(BlockBehaviour.Properties.of().destroyTime(3f).explosionResistance(3f).mapColor(MapColor.STONE).requiresCorrectToolForDrops().sound(SoundType.STONE).setId(ModUtil.createBlockResourceKey(name))));
	}

	public static DeferredBlock<Block> registerDeepslateOre(String name) {
		return registerBlockWithItem(name, () -> new Block(BlockBehaviour.Properties.of().destroyTime(4.5f).explosionResistance(3f).mapColor(MapColor.DEEPSLATE).sound(SoundType.DEEPSLATE).requiresCorrectToolForDrops().setId(ModUtil.createBlockResourceKey(name))));
	}
