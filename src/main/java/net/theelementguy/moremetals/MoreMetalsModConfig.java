...
public class MoreMetalsModConfig {
    ...
    private static final ModConfigSpec.ConfigValue<List<? extends String>> ITEM_STRINGS = BUILDER.comment("A list of items to log on common setup.").defineListAllowEmpty("items", List.of("minecraft:iron_ingot"), MoreMetalsModConfig::validateItemName);
    ...
    static void onLoad(final ModConfigEvent event) {
        ...
        items = ITEM_STRINGS.get().stream()
                .map(itemName -> BuiltInRegistries.ITEM.getValue(ResourceLocation.parse(itemName))).collect(Collectors.toSet());
    }
}
